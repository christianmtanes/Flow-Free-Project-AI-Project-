...
def resetVariable(variable, allcolors):
    ...
    return variable
'''
mrv that orders variable by their legal values
'''
def MRV(UnassignedVariables):
    UnassignedVariables.sort(key=lambda x: len(x.legalValues), reverse=False)
    ...
    while(i >= 0 and i < len(variables)):
        ...
        if value == None:
            ...
            if i < len(variables):
                ...
                UnassignedVariables = MRV(UnassignedVariables)
