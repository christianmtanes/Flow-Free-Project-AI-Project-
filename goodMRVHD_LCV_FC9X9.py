...
def constraints(variable, value):
    ...
    # here the suggest value is a vector (neighbor for previous, color , neighbor for next)
        ...
        return True
'''
a value ordering heuristic , return the value of the least constraining value on
other neighbors.
'''
def LCV(variable,allcolors):
    constrainsOnOthers = []
    i = 0
    while i < len(variable.legalValues):
        value = variable.legalValues[i]
        variable.setVariable(value)
        count = 0
        for neighbor in variable.getNeighbors():
            if not neighbor.isAssigned():
                count +=len(getConsistentDomain(neighbor, allcolors))
        constrainsOnOthers.append(count)
        i +=1
    maxIndex = constrainsOnOthers.index(max(constrainsOnOthers))
    return variable.legalValues[maxIndex]

'''
select a value to be assigned that is consistent
'''
def selectValue(variable,allcolors):
    if(len(variable.legalValues) != 0):
        value =  LCV(variable, allcolors)
        variable.legalValues.remove(value)
        return value
    return None
'''
get a domain that is always consistent with current assignment, instead of reinitializing to the original domain
'''
...
def resetVariable(variable, allcolors):
    ...
    return variable
'''
mrv that orders variable by their legal values and the number of unassigned neighbors
'''
def MRVH(UnassignedVariables):
    UnassignedVariables.sort(key=lambda x: (len(x.legalValues) + x.NumUnassignedNeighbors(), -x.NumUnassignedNeighbors()), reverse=False)
    return UnassignedVariables
def ForwardChecking(variable, allcolors):
    for var in variable.getNeighbors():
        if not var.isAssigned():
            var.legalValues = getConsistentDomain(var, allcolors)
            if len(var.legalValues) == 0:
                return False
    return True
...
def backtrack(variables,allcolors):
    ...
    while(i >= 0 and i < len(variables)):
        value = selectValue(variables[i],allcolors)
        if value == None:
            # backtracking
            variables[i] = resetVariable(variables[i], allcolors)
            i -= 1
            ...
            if i < len(variables):
                ...
                if not ForwardChecking(variables[i-1],allcolors):
                        i -= 1
                        if not(variables[i].isTarget or variables[i].isSource):
                            variables[i].color = '0'
                        variables[i].next = None
                        variables[i].previous = None
                else:
##                  mrv  is called here
                    UnassignedVariables = variables[i:]
                    for var in UnassignedVariables:
                        var.legalValues = getConsistentDomain(var, allcolors)
                    UnassignedVariables = MRVH(UnassignedVariables)
                    variables = variables[:i] + UnassignedVariables
                    variables[i]= resetVariable(variables[i], allcolors)
...
for game in Tests.goodMRV9X9games:
    allColors =  game[0]
    board = game[1]
    length = len(board)
